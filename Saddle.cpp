...
/* The R version of this function.
Saddle.r<-function(q,eg.values){
    x.hat<-Bisection(eg.values,q)
    
    w<-sign(x.hat)*sqrt(2*(x.hat*q-K.r(x.hat,eg.values)))
    v<-x.hat*sqrt(K2.r(x.hat,eg.values))
    pval<-1-pnorm(w+log(v/w)/w)
    return(pval)
}



K1.r<-function(x,eg.values,q){
    return(sum(eg.values/(1-2*eg.values*x))-q)
}

K.r<-function(x,eg.values){
    return(-0.5*sum(log(1-2*eg.values*x)))
}

K2.r<-function(x,eg.values){
    return(2*sum(eg.values^2/(1-2*eg.values*x)^2))
}
*/
...
/* ####   This part is from the R package "CompQuadForm"      ##### */
